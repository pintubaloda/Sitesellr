...
public class PlatformThemesController : ControllerBase
{
    ...
    public async Task<IActionResult> ImportThemeZip([FromForm] IFormFile file, CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(manifest.Slug)) return BadRequest(new { error = "manifest_slug_required" });
        ...
        {
            return BadRequest(new { error = "zip_invalid" });
        }
        catch (JsonException)
        {
            return BadRequest(new { error = "manifest_invalid_json" });
        }
        catch (Exception ex)
        {
            return StatusCode(StatusCodes.Status500InternalServerError, new { error = "theme_import_failed", detail = ex.Message });
        }
    }
